...
*/
SELECT CONCAT(name, ' (', region, ')') AS city_region FROM cities WHERE population > 100000 ORDER BY name ASC;

/*
	Отримати перші 50 міст України (за населенням) та додати до запити обчислюване поле в якому буде вказано який процент від усього населення країни проживає у цьому місті
	За загальну кількість населення України беремо число - 40 000 000
*/
SELECT name, population, ROUND((population / 40000000) * 100, 2) AS percent_population FROM cities ORDER BY population DESC LIMIT 50;
...
*/
SELECT CONCAT(name, ' - ', ROUND((population / 40000000) * 100, 2), '%') AS city_population_percent FROM cities WHERE population >= 40000 ORDER BY population DESC;
